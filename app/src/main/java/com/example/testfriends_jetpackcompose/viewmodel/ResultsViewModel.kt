...
class ResultsViewModel @Inject constructor(
    ...
    fun challenge(id: Int) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            try {
                var response = resultsRepo.challenge(id)
                if (response.isSuccessful) {
                    challenge.value = NetworkResults.Success(response.body()!!)
                    Log.d("question", challenge.value.data!!.username)
                }
            } catch (ex: Exception) {
                challenge.value = NetworkResults.Error(message = ex.message)
            }
